...
def _format_results(extracted_sentences, split, score, num_sentence):
    if score:
...
def summarize(text, num_sent=2, words=None, sent_count=None, language="english", split=False, scores=False, additional_stopwords=None):
    ...
    extracted_sentences = _extract_most_important_sentences(sentences, sent_count, words)
    # Sorts the extracted sentences by apparition order in the original text.
    extracted_sentences.sort(key=lambda s: s.index)

    return _format_results(extracted_sentences, split, scores, sent_count)
